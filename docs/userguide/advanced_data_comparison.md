...
- `to_have_count(a_items ut_varchar2)` -  Items to be included in count

- `to_have_count(a_items ut_varchar2_list)` -  table of items to be included in count
...
### Counting elements of json

`to_have_count` supports json objects,scalars and arrays. For json objects it will return a number of  keys, for arrays number of elements and for scalar always 1. As an argument into matcher you can pass a json path to element that will be count. If this is null it will take a current object.
