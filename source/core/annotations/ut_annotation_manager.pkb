create or replace package body ut_annotation_manager as
  ...
  */
  ...
  begin
    ...
    ut_event_manager.trigger_event( 'get_objects_to_refresh - l_refresh_needed' ||ut_utils.to_string(l_refresh_needed ) );
    if l_refresh_needed then
      ...
      ut_event_manager.trigger_event('get_objects_to_refresh (l_cached_objects.count='||l_cached_objects.count||')');
      execute immediate
