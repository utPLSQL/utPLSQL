create or replace package body ut_expectation_processor as
  ...
  */
  ...
  g_asserts_called ut_expectation_results := ut_expectation_results();
  ...
  begin
    ut_utils.debug_log('ut_expectation_processor.get_aggregate_asserts_result');

    for i in 1 .. g_asserts_called.count loop
      l_result := greatest(l_result, g_asserts_called(i).status);
  ...
  begin
    ut_utils.debug_log('ut_expectation_processor.clear_asserts');
    g_asserts_called.delete;
  end;

  function get_asserts_results return ut_expectation_results is
    l_asserts_results ut_expectation_results := ut_expectation_results();
  begin
    ut_utils.debug_log('ut_expectation_processor.get_asserts_results: .count='||g_asserts_called.count);
    l_asserts_results := g_asserts_called;
    clear_asserts();
    return l_asserts_results;
  end get_asserts_results;

  procedure add_assert_result(a_assert_result ut_expectation_result) is
  begin
    ut_utils.debug_log('ut_expectation_processor.add_assert_result');
  ...
  begin
    add_assert_result(ut_expectation_result(ut_utils.tr_failure, null, a_message));
  end;
