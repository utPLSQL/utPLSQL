  create or replace package body ut_suite_manager is
    ...
    */
    ...
    function config_package(a_owner_name varchar2, a_object_name varchar2) return ut_logical_suite is
      l_annotation_data    ut_annotations;
    ...
    begin
      ...
      for i in 1 .. l_annotation_data.count loop

        if l_annotation_data(i).subobject_name is null then

          if l_annotation_data(i).name in ('suite','displayname') then
            l_suite_name := l_annotation_data(i).text;
            if l_annotation_data(i).name = 'suite' then
              l_is_suite := true;
            end if;
          elsif l_annotation_data(i).name = 'disabled' then
            l_suite_disabled := true;
          elsif l_annotation_data(i).name = 'suitepath' and  l_annotation_data(i).text is not null then
            l_suite_path := l_annotation_data(i).text || '.' || lower(l_object_name);
          elsif l_annotation_data(i).name = 'rollback' then
            if lower(l_annotation_data(i).text) = 'manual' then
          ...
          l_proc_name := l_annotation_data(i).subobject_name;

          if l_annotation_data(i).name = 'beforeeach' and l_default_setup_proc is null then
            l_default_setup_proc := l_proc_name;
          elsif l_annotation_data(i).name = 'aftereach' and l_default_teardown_proc is null then
            l_default_teardown_proc := l_proc_name;
          elsif l_annotation_data(i).name = 'beforeall' and l_suite_setup_proc is null then
            l_suite_setup_proc := l_proc_name;
          elsif l_annotation_data(i).name = 'afterall' and l_suite_teardown_proc is null then
            l_suite_teardown_proc := l_proc_name;


          elsif l_annotation_data(i).name = 'disabled' then
            l_test_disabled := true;
          elsif l_annotation_data(i).name = 'beforetest' then
            l_beforetest_procedure := l_annotation_data(i).text;
          elsif l_annotation_data(i).name = 'aftertest' then
            l_aftertest_procedure := l_annotation_data(i).text;
          elsif l_annotation_data(i).name in ('displayname','test') then
            l_displayname := l_annotation_data(i).text;
            if l_annotation_data(i).name = 'test' then
              l_is_test := true;
            end if;
          elsif l_annotation_data(i).name = 'rollback' then
            if lower(l_annotation_data(i).text) = 'manual' then
              l_rollback_type := ut_utils.gc_rollback_manual;
            elsif lower(l_annotation_data(i).text) = 'auto' then
              l_rollback_type := ut_utils.gc_rollback_auto;
            end if;
          end if;

          if l_is_test
             and (i = l_annotation_data.count or l_proc_name != nvl(l_annotation_data(i+1).subobject_name, ' ') ) then
            l_suite_items.extend;
            l_suite_items(l_suite_items.last) :=
              ut_test(a_object_owner          => l_owner_name
