create or replace type body be_less_or_equal AS

  member procedure init(self in out nocopy be_less_or_equal, a_expected ut_data_value) is
  begin
    self.name := lower($$plsql_unit);
    self.expected := a_expected;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected date) return self as result is
  begin
    init(ut_data_value_date(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected NUMBER) return self as result is
  begin
    init(ut_data_value_number(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected timestamp_unconstrained) return self as result is
  begin
    init(ut_data_value_timestamp(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected timestamp_tz_unconstrained) return self as result is
  begin
    init(ut_data_value_timestamp_tz(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected timestamp_ltz_unconstrained) return self as result is
  begin
    init(ut_data_value_timestamp_ltz(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected yminterval_unconstrained) return self as RESULT IS
  begin
    init(ut_data_value_yminterval(a_expected));
    return;
  end;

  constructor function be_less_or_equal(self in out nocopy be_less_or_equal, a_expected dsinterval_unconstrained) return self as RESULT IS
  begin
    init(ut_data_value_dsinterval(a_expected));
    return;
  end;

  overriding member function run_matcher(self in out nocopy be_less_or_equal, a_actual ut_data_value) return boolean is
    l_result boolean;
  begin
