create or replace type body ut_expectation as
  ...
  begin
    ...
    l_assert_name   := 'to '||l_matcher.name;
    if l_matcher.expected is not null then
      add_assert_result( l_assert_result, l_assert_name, l_matcher.additional_info, l_matcher.error_message
        , l_matcher.expected.to_string(), l_matcher.expected.type );
    else
      add_assert_result( l_assert_result, l_assert_name, l_matcher.additional_info, l_matcher.error_message );
    end if;
  end;
  ...
  begin
    ...
    l_assert_name   := 'not to '||l_matcher.name;
    if l_matcher.expected is not null then
      add_assert_result( l_assert_result, l_assert_name, l_matcher.additional_info, l_matcher.error_message
        , l_matcher.expected.to_string(), l_matcher.expected.type );
    else
