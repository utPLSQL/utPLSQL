create or replace type body ut_cursor_details as
  ...
  member procedure get_anytype_members_info(a_anytype anytype, a_attribute_typecode out pls_integer,
    ...
    l_csfrm              pls_integer;
  begin
    a_attribute_typecode := a_anytype.getinfo(
      prec        => l_prec,
      scale       => l_scale,
      len         => a_len,
      csid        => l_csid,
  ...
  begin
    a_attribute_typecode := a_anytype.getattreleminfo(
      pos           => a_pos, --First attribute
      prec          => l_prec,
      scale         => l_scale,
      len           => a_len,
      csid          => l_csid,
  ...
  begin
    ...
    **/
    for cur in 1 .. l_columns_count loop
      l_is_collection := ut_compound_data_helper.is_collection(
      ...
      if l_columns_desc(cur).col_type = dbms_sql.user_defined_type or  l_is_collection then
        ...
        l_anydata := get_anydata_from_compound_data(
          l_columns_desc(cur).col_schema_name,
          l_columns_desc(cur).col_type_name,
          l_object_type
        );
