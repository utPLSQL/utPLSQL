create or replace type body ut_dbms_output_suite_reporter is
  ...
  begin
    self.print_text(ut_dbms_output_suite_reporter.c_dashed_line);
    self.print_text('suite "' || a_suite.name || '" started.');
  end;
  ...
  begin
    ...
    if a_test.name is not null then
      self.print_text('test  ' || test.name || ' (' || test.test.form_name || ')');
    else
      self.print_text('test  ' || test.test.form_name);
    end if;
  end;
  ...
  begin
    if l_assert is not null then
      if l_assert.message is not null then
        self.print_text('message: '||l_assert.message);
      end if;
      if l_assert.result != ut_utils.tr_success then
        self.print_text('expected: ' || l_assert.actual_value_string||'('||l_assert.actual_type||')');
        self.print_text(
        ...
        self.print_text(l_assert.result_to_char());
      end if;
      if l_assert.error_message is not null then
        self.print_text('error message: '||l_assert.error_message);
      end if;
    end if;
  end;
  ...
  begin
    self.print_text('result: ' || l_test.result_to_char||'. Took: '||to_char(l_duration));
  end;
  ...
  begin
    ...
    self.print_text(ut_dbms_output_suite_reporter.c_dashed_line);
    self.print_text('suite "' || l_suite.name || '" ended. Took: '||to_char(l_duration));
    self.print_text(ut_dbms_output_suite_reporter.c_dashed_line);
  end;
