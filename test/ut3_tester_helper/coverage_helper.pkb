create or replace package body coverage_helper is
  ...
  begin
    select nvl(min(runid),0) - 1 into v_result
      from ut3_develop.plsql_profiler_runs;
    return v_result;
  end; 
  ...
  begin
    ...
    values(g_profiler_run_id, c_unit_id, 'PACKAGE BODY', 'UT3_DEVELOP', 'DUMMY_COVERAGE');
  ...
  procedure drop_dummy_coverage_test_1 is
    pragma autonomous_transaction;
  begin
    execute immediate q'[drop package UT3_DEVELOP.DUMMY_COVERAGE_1]';
    execute immediate q'[drop package UT3_DEVELOP.TEST_DUMMY_COVERAGE_1]';
  end;
